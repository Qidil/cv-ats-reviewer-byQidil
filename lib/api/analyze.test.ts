...
function openRouter(reply: (mode: AnalysisMode) => Response = (mode) =>
  ...
  const fetchMock = vi.fn(async (_url: string | URL | Request, init?: RequestInit) => {
    ...
    return reply(body.includes("suggestedJobs berisi") ? "mode-b" : "mode-a");
  });
...
function deps(overrides: Partial<AnalyzeDependencies> = {}): AnalyzeDependencies & { quotaStore: QuotaStore } {
  ...
  return { config: CONFIG, quotaStore, fetch: openRouter().fetch, ...overrides } as AnalyzeDependencies & {
...
describe("runAnalysis", () => {
  it("returns one merged Mode A report and never sends hidden text to the AI (BR-02)", async () => {
    ...
    expect(result.meta).toMatchObject({ modelUsed: "openrouter/free", failoverOccurred: false, quota: { limit: 10, used: 1, remaining: 9 } });
  ...
  it("answers Mode B in one request with job suggestions and counts it once (P3-D3)", async () => {
    ...
    expect(result.suggestedJobs).toHaveLength(1);
  ...
  it("uses a personal key and its model first, and never counts or blocks it (BR-10, AC-05.2)", async () => {
    ...
    const result = await runAnalysis(
      input({ authorization: "Bearer user-key", customModel: "anthropic/claude-3.5-haiku" }),
      dependencies,
    );

    expect(result.meta).toMatchObject({ modelUsed: "anthropic/claude-3.5-haiku", quota: null });
    expect(ai.authorizations).toEqual(["Bearer user-key"]);
  ...
  it("blocks the 11th built-in-key analysis of the day before calling the AI (BR-11)", async () => {
    ...
    expect(error).toMatchObject({ code: "DAILY_QUOTA_EXCEEDED", resetsAt: expect.stringMatching(/T00:00:00\+07:00$/) });
  ...
  it("limits every network per clock hour with any key, before extraction (rules.md §4.3.6)", async () => {
    let clock = Date.parse("2026-09-26T07:10:00Z"); // 14:10 WIB
    const ai = openRouter();
    const dependencies = deps({
    ...
    await runAnalysis(input(), dependencies);
    await runAnalysis(input(), dependencies);
    await runAnalysis(input({ authorization: "Bearer user-key" }), dependencies);

    // Not a PDF: reaching extraction would answer PDF_INVALID instead.
    const notPdf = new Blob(["bukan pdf"]);
    for (const overrides of [{ authorization: "Bearer user-key", file: notPdf }, { file: notPdf }]) {
    ...
    await expect(runAnalysis(input({ authorization: "Bearer user-key" }), dependencies)).resolves.toBeDefined();
  });
  ...
  it("skips the hourly limit during a store outage, so a personal key still works", async () => {
    ...
    await expect(runAnalysis(input({ authorization: "Bearer user-key" }), deps({ quotaStore: down }))).resolves.toBeDefined();
  });
  ...
  it.each<[string, Partial<AnalyzeDependencies>]>([
    ...
    await expect(runAnalysis(input({ authorization: "Bearer user-key" }), deps(overrides))).resolves.toBeDefined();
  });
